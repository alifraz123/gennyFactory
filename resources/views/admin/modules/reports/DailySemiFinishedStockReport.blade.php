...
<div style="display:flex;justify-content:center">
    <p style="font-weight: bold;font-size:20px" >Genny Semi Finished Stock Report (Qty in pcs) {{$semifinish[0]->date}}</p>
</div>
...
<div style="height: 1000px; border:1px solid black; display:flex;flex-direction:column;flex-wrap:wrap;align-content:flex-start" class="">
    ...
    @for($b=0; $b < count($semifinish); $b++)


        <span style="background-color: black; color: white;text-align: center;width:10%">
            {{$semifinish[$b]->itemname}}
        </span>

        @for($c=0; $c < count($semifinish[$b]->varient); $c++)
            <div style="width: 10%; border:1px solid black">
                <div style="display: inline-block;width: 41px;text-align: center;">{{$semifinish[$b]->varient[$c]['varient']}}</div>
                <div style="display: inline-block;width: 45px;text-align: center;">{{$semifinish[$b]->varient[$c]['qty']}}</div>
            </div>

            @endfor
            <div style="width: 10%; border:1px solid black">
                <div style="display: inline-block;width: 41px;text-align: center;font-weight:bold;">Total</div>
                <div style="display: inline-block;width: 45px;text-align: center;font-weight:bold;">{{$semifinish[$b]->total}}</div>
            </div>
